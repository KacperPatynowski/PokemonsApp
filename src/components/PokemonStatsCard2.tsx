...
import { usePokemonQuery } from "services/api/usePokemonQuery";
...
export const PokemonStatsCard2 = ({ pokemonId }: IProps) => {
  const {
    data: response,
    isLoading,
    isSuccess,
  ...
  // const pokemonsArray2 = [evolutionChain?.species.name];

  const {
    data: pokemonsResponse,
    isLoading: statsLoading,
    isSuccess: pokemonSuccess,
  } = usePokemonQuery(pokemonId.toString());
  if (pokemonSuccess) {
    const { id, name, sprites, types, stats } = pokemonsResponse!.data || {};
...
//   usePokemonsStatsQuery(pokemonsArray2);
